							...
							<div class="col-xs-12">
								...
								<form id="form" method="post" action="<?php echo U('del');?>">
									...
									<?php if(is_array($userList)): $i = 0; $__LIST__ = $userList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                                        	<td><img src="<?php echo ($val['headimgurl']); ?>" width="35" height="35"></td>
											<td style="line-height:30px"><?php echo ($val['wxname']); ?></td>
                                            <td style="text-align:center; line-height:30px"><?php if($val['gender'] != 1 ): ?>女<?php else: ?>男<?php endif; ?></td>
											<td style="line-height:30px"><?php echo ($val['tel']); ?></td>
											<td style="line-height:30px"><?php echo (date("Y-m-d H:i:s",$val['dateline'])); ?></td>
